...
//    static const int    leafslots = MIN_BTREE_LEAF_NUM;
...
public:
    ...
    struct inner_node : public node
    {
        ...
        typedef typename std::allocator_traits<_Alloc>::template rebind_alloc<inner_node> alloc_type;
...
//        typedef typename _Alloc::template rebind<leaf_node>::other alloc_type;
        typedef typename std::allocator_traits<_Alloc>::template rebind_alloc<leaf_node> alloc_type;
...
//	LRU_Manager_t LRU;
	...
	// DIY functions
//	void initDisk() {
//		data_type pids[pageSize];
//		key_type keys[pageSize];
//		int sz = 0;
//		pageIdx = 0;
//		if (m_root != NULL) {
//			__initDisk(m_root, pids, keys, sz);
//			if (sz > 0) {
//				// dump to File
//				assert(sz <= pageSize);
//				dumpToDisk(pageIdx++, pids, keys, pageSize);
//			}
//		}
//	}
	
//	void __initDisk(node *n, data_type* pids, key_type* keys, int& sz) {
//		if (n->isleafnode())
//        {
//			leaf_node *leafnode = static_cast<leaf_node*>(n);
//			for (int i=0; i<leafnode->slotuse; ++i) {
//				assert((leafnode->idx+i)/pageSize == pageIdx);
//				pids[sz] = leafnode->slotdata[i];
//				keys[sz] = leafnode->slotkey[i];
//				++sz;
//				if (sz == pageSize) {
//					// dump to File
//					dumpToDisk(pageIdx++, pids, keys, pageSize);
...
//            {
//				__initDisk(innernode->childid[slot], pids, keys, sz);
//			}
//		}
//	}
...
//
//    template <class type>
//    void initDisk(Dataset<type>* dataset)
//    {
//
//        size_t _memory_cost = 0, leaf_cost, sizePage = 0, pageCost = 0;
...
//        {
//
//            it->memory_cost = new size_t[it->slotuse];
//
//            for(size_t i = 0; i < it->slotuse; ++i)
//            {
//
//                leaf_cost = sizeof(size_t) + dataset->getFeatureVector(it->slotdata[i]).getSerializedSize();
//
////                cout << it->slotdata[i] << " ";
//
//                if(((size_t)(_memory_cost + leaf_cost + sizeof(size_t))/PAGE_SIZE) == pageIdx)
//                {
//
//                    _memory_cost += leaf_cost;
//                    it->memory_cost[i] = _memory_cost;
//                    pageCost += leaf_cost;
//                    ids.push_back(it->slotdata[i]);
//                    sizePage++;
//
//                }
//                else
...
//                    //dump to disk
//                    //IOwrite++;
//                    //cout << "PAGE SIZE = " << sizePage << endl;
//                    write_to_disk(dataset, ids, pageCost, sizePage, pageIdx);
//                    pageIdx++;
//
//                    sizePage = 1;
//                    _memory_cost += leaf_cost;
...
//                    //dump to disk
//                    IOwrite++;
//                    write_to_disk(dataset, last, --it, sizePage, pageIdx);
//                    pageIdx++;
//                    it++;
//                    sizePage = 0;
...
//            leaf_cost = sizeof(size_t) + dataset->getFeatureVector(it.data()).getSerializedSize();
...
//    size_t getPageSize(size_t mem)
//    {
//
//        return (size_t)((mem + sizeof(size_t))/PAGE_SIZE);
//
//    }
//
...
//        //_init_RAF_ID(0, m_root);
//    }
	
//    int _init_RAF_ID(int pid, node *n) {
////        if (n->isleafnode())
////        {
////            leaf_node *leafnode = static_cast<leaf_node*>(n);
////            leafnode->idx = pid;
////            return leafnode->slotuse;
////        }
////        else
////        {
////            int ret = 0;
...
////            for (int slot = 0; slot < innernode->slotuse + 1; ++slot)
////            {
////                ret += _init_RAF_ID(pid+ret, innernode->childid[slot]);
////            }
////            return ret;
////        }
...
//            size_t memory_cost = sizeof(size_t);

//        }

//    }
	
//    void dump_Key_Minmax() {
//        _dump_Key_Minmax(m_root, 0);
//    }
//
...
//            {
//                _dump_Key_Minmax(innernode->childid[slot], depth+1);
//            }
//        }
//    }
	
//	void updateBoundary(int ba[][2], unsigned long long* coord) {
//		for (int i=0; i<PIVOT_N; ++i) {
//			ba[i][0] = min(ba[i][0], (int)coord[i]);
//			ba[i][1] = max(ba[i][1], (int)coord[i]);
//		}
//	}
	
//	void updateBoundaryBySFC(int ba[][2], key_type SFC_value) {
//		unsigned long long coord[PIVOT_N];
//		hilbert_i2c(PIVOT_N, SFC_nBits, SFC_value, coord);
//		reverse(coord, coord+PIVOT_N);
//		updateBoundary(ba, coord);
//	}
...
//        cout << "LEAFS = " << m_stats.leaves << endl;
//        cout << "AVG LEAFS = " << m_stats.leaves << endl;

//        vector<size_t> v;

//        leaf_node *leafnode = static_cast<leaf_node*>(m_headleaf);

//        cout << endl;
//        for(size_t i = 0; i < leafnode->slotuse; i++)
//        {

//            cout << leafnode->slotdata[i] << " / " << leafnode->slotkey[i] << endl;
//            v.push_back(leafnode->slotdata[i]);
...
//            cout << leafnode->slotdata[i] << " / " << leafnode->slotkey[i] << endl;
//            if(std::find(v.begin(), v.end(), leafnode->slotdata[i]) != v.end())
//            {

//                cout << "SAME ELEMENT : " << leafnode->slotdata[i] << endl;
...
//        cout << leafnode->key_min << " / " << leafnode->key_max << endl;
//        cout << leafnode->mbr[0][0] << " / " << leafnode->mbr[0][1] << endl;
//        cout << leafnode->mbr[1][0] << " / " << leafnode->mbr[1][1] << endl << endl;
...
//            cout << "[ ";
//            for(ull pt : hc.point_from_distance(k))
//                cout << pt << " ";
//            cout << "]\n";
...
//        cout << endl;
//        cout << m_stats.nodes() << endl;
//        cout << m_stats.leaves << endl;


//    }
//
//    void _init_Key_Minmax(node *n)
//    {
//
//        if(n->isleafnode())
//        {
...
//            {
//
//                leafnode->key_min = std::min(leafnode->key_min, leafnode->slotkey[i]);
//                leafnode->key_max = std::max(leafnode->key_max, leafnode->slotkey[i]);
//                keys.push_back(leafnode->slotkey[i]);
//
//            }
//
//            leafnode->mbr = new ull*[PIVOT_NUM];
//
//            for (int i=0; i<PIVOT_NUM; ++i)
//            {
//
//                leafnode->mbr[i] = new ull[2];
//                leafnode->mbr[i][0] = GRID_L;
//                leafnode->mbr[i][1] = (ull)0;
//
//            }
//
...
//                {
//
//                    leafnode->mbr[i][0] = std::min(leafnode->mbr[i][0], points[j][i]);
//                    leafnode->mbr[i][1] = std::max(leafnode->mbr[i][1], points[j][i]);
//
//                }
//
//            }
...
//            for (int slot = 0; slot < innernode->slotuse + 1; ++slot)
//            {
//                _init_Key_Minmax(innernode->childid[slot]);
...
//            }
//
//            innernode->mbr = new ull*[PIVOT_NUM];
//
//            for (int i=0; i < PIVOT_NUM; ++i){
//
//                innernode->mbr[i] = new ull[2];
//                innernode->mbr[i][0] = GRID_L;
//                innernode->mbr[i][1] = (ull)0;
//
//            }
//
//            for (int slot = 0; slot < innernode->slotuse + 1; ++slot) {
...
//                {
//
//                    innernode->mbr[i][0] = std::min(innernode->mbr[i][0], innernode->childid[slot]->mbr[i][0]);
//                    innernode->mbr[i][1] = std::max(innernode->mbr[i][1], innernode->childid[slot]->mbr[i][1]);
//
//                }
//
//            }
...
//        ull coord[PIVOT_NUM];
//        ull cornerlo[PIVOT_NUM];
//        ull cornerhi[PIVOT_NUM];
//        ull work[PIVOT_NUM];
...
//            for (int i=0; i<leafnode->slotuse; ++i) {
//                leafnode->key_min = min(leafnode->key_min, leafnode->slotkey[i]);
//                leafnode->key_max = max(leafnode->key_max, leafnode->slotkey[i]);
//            }
...
//            }
//            for (int i=0; i<leafnode->slotuse; ++i) {
//                updateBoundaryBySFC(leafnode->mbr, leafnode->slotkey[i]);
//            }
//            for (int i=0; i<PIVOT_NUM; ++i) {
//                cornerlo[i] = leafnode->mbr[i][0];
//                work[i] = leafnode->mbr[i][1];
//            }
//            hilbert_box_pt(PIVOT_NUM, sizeof(unsigned long long), 8*sizeof(unsigned long long), 1, cornerlo, work);
//            for (int i=0; i<PIVOT_NUM; ++i) {
//                work[i] = leafnode->mbr[i][0];
//                cornerhi[i] = leafnode->mbr[i][1];
//            }
//            hilbert_box_pt(PIVOT_NUM, sizeof(unsigned long long), 8*sizeof(unsigned long long), 0, work, cornerhi);
//            unsigned long long SFC_value;
//            SFC_value = hilbert_c2i(PIVOT_NUM, p, cornerlo);
//            leafnode->key_min = min(leafnode->key_min, SFC_value);
//            leafnode->key_max = max(leafnode->key_max, SFC_value);
//            SFC_value = hilbert_c2i(PIVOT_NUM, p, cornerhi);
//            leafnode->key_min = min(leafnode->key_min, SFC_value);
//            leafnode->key_max = max(leafnode->key_max, SFC_value);
//        }
//        else
...
//            {
//                _init_Key_Minmax(innernode->childid[slot]);
//                innernode->key_min = min(innernode->key_min, innernode->childid[slot]->key_min);
//                innernode->key_max = max(innernode->key_max, innernode->childid[slot]->key_max);
//            }
...
//                for (int i=0; i < PIVOT_NUM; ++i) {
//                    innernode->mbr[i][0] = min(innernode->mbr[i][0], innernode->childid[slot]->mbr[i][0]);
//                    innernode->mbr[i][1] = max(innernode->mbr[i][1], innernode->childid[slot]->mbr[i][1]);
//                }

//            }

//            for (int i=0; i<PIVOT_NUM; ++i) {
//                cornerlo[i] = innernode->mbr[i][0];
//                work[i] = innernode->mbr[i][1];
//            }

//            hilbert_box_pt(PIVOT_N, sizeof(unsigned long long), 8*sizeof(unsigned long long), 1, cornerlo, work);
//            for (int i=0; i<PIVOT_N; ++i) {
//                work[i] = innernode->mbr[i][0];
//                cornerhi[i] = innernode->mbr[i][1];
//            }
//            hilbert_box_pt(PIVOT_N, sizeof(unsigned long long), 8*sizeof(unsigned long long), 0, work, cornerhi);
//            // if (innernode->slotuse == 2) {
//            // printf("(");
//            // for (int i=0; i<PIVOT_N; ++i) {
//            // if (i+1 == PIVOT_N) {
//            // printf("%d", innernode->mbr[i][0]);
//            // } else {
//            // printf("%d,", innernode->mbr[i][0]);
//            // }
//            // }
...
//            unsigned long long SFC_value;
//            SFC_value = hilbert_c2i(PIVOT_NUM, p, cornerlo);
//            innernode->key_min = min(innernode->key_min, SFC_value);
//            innernode->key_max = max(innernode->key_max, SFC_value);
//            SFC_value = hilbert_c2i(PIVOT_N, SFC_nBits, cornerhi);
//            innernode->key_min = min(innernode->key_min, SFC_value);
//            innernode->key_max = max(innernode->key_max, SFC_value);
//        }
//    }
...
public:
	// DIY functions
    /// range query with hooked check function
//	bool intersect(int mbr[][2], int qmbr[][2]) {
//		for (int i=0; i<PIVOT_N; ++i) {
//			if (mbr[i][1]<qmbr[i][0] || mbr[i][0]>qmbr[i][1])
//				return false;
//		}
//		return true;
//	}
	
//	bool intersect(node *p, int qmbr[][2]) {
//		return intersect(p->mbr, qmbr);
//	}
	
//	bool intersect(key_type SFC_value, int qmbr[][2]) {
//		unsigned long long coord[PIVOT_N];
//		hilbert_i2c(PIVOT_N, SFC_nBits, SFC_value, coord);
//		reverse(coord, coord+PIVOT_N);
//		int mbr[PIVOT_N][2];
//		for (int i=0; i<PIVOT_N; ++i) {
//			mbr[i][0] = mbr[i][1] = coord[i];
//		}
//		return intersect(mbr, qmbr);
//	}
	
//	void _searchLeafNode_RQ(node *p, int qmbr[][2], vector<data_type>& answers, location_t& qloc, double radius) {
//		leaf_node *leafnode = static_cast<leaf_node*>(p);
//		location_t *locs;
//		unsigned long long *keys;
		
//		for (int i=0; i<leafnode->slotuse; ++i) {
//			LRU.access(leafnode->idx+i, locs, keys);
//			int pos = LRU.calcPagePos(leafnode->idx+i);
//			if (intersect(keys[pos], qmbr)) {
//				if (dist(locs[pos], qloc, tag) <= radius)
//					answers.push_back(leafnode->slotdata[i]);
//			}
//		}
//	}
	
	
//    void rangeQuery(location_t& qloc, double radius, vector<data_type>& answers, int qmbr[][2])
//    {
//        node *p = m_root;
//		std::queue<node*> Q;
...
//			}
//			if (p->isleafnode()) {
//				_searchLeafNode_RQ(p, qmbr, answers, qloc, radius);
//			} else {
//				inner_node *innernode = static_cast<inner_node*>(p);
//				for (unsigned short slot = 0; slot < innernode->slotuse + 1; ++slot)
//				{
//					Q.push(innernode->childid[slot]);
//				}
//			}
...
//	typedef pair<double,node*> qpair_t;
//	void _searchLeafNode_KnnQ(priority_queue<pair<double,data_type>, vector<pair<double,data_type> >, less<pair<double,data_type> > >& H, double& dk, node *p, location_t& qloc, int kth) {
//		leaf_node *leafnode = static_cast<leaf_node*>(p);
//		location_t *locs;
//		unsigned long long *keys;
...
//			LRU.access(leafnode->idx+i, locs, keys);
//			int pos = LRU.calcPagePos(leafnode->idx+i);
//			double dis = dist(locs[pos], qloc, tag);
//			if (H.size() < kth) {
//				H.push(make_pair(dis, leafnode->slotdata[i]));
//			} else if (dis < H.top().first){
//				H.pop();
//				H.push(make_pair(dis, leafnode->slotdata[i]));
//			}
//			if (H.size() >= kth) {
//				dk = min(dk, H.top().first);
//			}
//		}
//	}
	
	
//    /// knn query with hooked check function
//    void knnQuery(location_t& qloc, int kth, vector<data_type>& answers, double (*MIND)(int[][2]))
//    {
//		answers.clear();
//		if (kth == 0) return;
		
//		priority_queue<qpair_t, vector<qpair_t>, greater<qpair_t> > Q;
//		priority_queue<pair<double,data_type>, vector<pair<double,data_type> >, less<pair<double,data_type> > > H;
//		node *p = m_root;
//		qpair_t cur;
//		int pid, _kth = kth;
//		double dk = GRID_L*2.0;
		
//		if (p != NULL) {
//			cur = make_pair(MIND(p->mbr), p);
//			Q.push(cur);
//		}
...
//			cur = Q.top();
//			Q.pop();
//			if (cur.first > dk) break;
//			p = cur.second;
			
//			if (p->isleafnode()) {
//				_searchLeafNode_KnnQ(H, dk, p, qloc, kth);
//			} else {
//				inner_node *innernode = static_cast<inner_node*>(p);
//				for (unsigned short slot = 0; slot < innernode->slotuse + 1; ++slot)
//				{
//					double disTmp = MIND(innernode->childid[slot]->mbr);
//					if (disTmp <= dk) {
//						Q.push(make_pair(disTmp, innernode->childid[slot]));
//					}
...
//        while (!H.empty()) {
//			answers.push_back(H.top().second);
//			H.pop();
//		}
...
public:
    ...
    void _init_Key_Minmax(node *n) {
        ...
        if (n->isleafnode()) {
            ...
            innernode->mbr = new ull *[numPivots];
