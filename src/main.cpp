...
int main(int argc, char **argv)
{
    ...
    std::unique_ptr<kdtree::KdTree<size_t, double>> kd = std::make_unique<kdtree::KdTree<size_t, double>>(std::move(data1), std::move(dist1), std::move(pvt), 2, 500, 8000, false, true, true, "tmp_unit_test11");
    ...
    for(size_t i = 0; i < test->getCardinality(); i++)
    {
        ...
        for(size_t j = 0; j < res1.size(); j++)
        {
            ...
            std::cout << kd->getPrunning() << std::endl;
        }
    }
