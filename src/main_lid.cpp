...
int main(int argc, char **argv)
{
    ...
    std::unique_ptr<Dataset<size_t, double>> data1 = std::make_unique<Dataset<size_t, double>>(program.get<std::string>("-DATASET"), program.get<std::string>("-DATASET_SEPARATOR")),
            data2 = std::make_unique<Dataset<size_t, double>>(program.get<std::string>("-DATASET"), program.get<std::string>("-DATASET_SEPARATOR"));
    ...
    std::unique_ptr<Index<size_t, double>> index = std::make_unique<gervLib::index::vptree::VPTree<size_t, double>>(std::move(data1), std::move(distance_function), std::move(pivot),
            ...
            program.get<bool>("-STORE_DIRECTORY_NODE"), program.get<bool>("-STORE_LEAF_NODE"), program.get<bool>("-USE_LAESA"));

    std::unique_ptr<DistanceFunction<BasicArrayObject<size_t, double>>> df = gervLib::distance::DistanceFactory<BasicArrayObject<size_t, double>>::createDistanceFunction(index->getDistanceFunction()->getDistanceType());

    splitByLID(data2, df, index, std::stoul(program.get<std::string>("-K_MAX")), program.get<std::string>("-FILE_NAME"), program.get<std::string>("-OUTPUT_PATH"));

    std::filesystem::path path(program.get<std::string>("-OUTPUT_PATH"));

    for (int i = 0; i < 4; i++)
    {

        std::filesystem::path file_name = path / (program.get<std::string>("-FILE_NAME") + "_Q" + std::to_string(i+1) + ".csv");
        splitTrainTest<size_t, double>(file_name, ",", 0.9, std::stoul(program.get<std::string>("-SEED")));
