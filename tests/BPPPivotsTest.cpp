...
int test1()
{
    ...
    auto pvt = std::make_unique<BPPPivots<size_t, double>>();
    pvt->setSeed(43);
    pvt->operator()(data, dist, 4);
    std::unique_ptr<u_char[]> arr = pvt->serialize();
    std::unique_ptr<Pivot<size_t, double>> pvt2 = std::make_unique<BPPPivots<size_t, double>>();
...
int test2()
{
    ...
    assert(pvt->getPivot(2).getOID() == 14);
    ...
    assert(pvt->getPivot(2).getOID() == 17);
