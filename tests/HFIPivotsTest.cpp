...
int test1()
{
    ...
    auto pvt = std::make_unique<HFIPivots<size_t, double>>();
    pvt->setSeed(572);
    pvt->operator()(data, dist, 4);
    std::unique_ptr<u_char[]> arr = pvt->serialize();
    std::unique_ptr<Pivot<unsigned long, double>> pvt2 = std::make_unique<HFIPivots<size_t, double>>();
...
int test2()
{
    ...
    for (size_t i = 0; i < data->getCardinality(); i++)
        data->operator[](i).setOID(data->operator[](i).getOID()+3);
    std::unique_ptr<DistanceFunction<BasicArrayObject<size_t, double>>> dist = std::make_unique<EuclideanDistance<BasicArrayObject<size_t, double>>>();
    auto pvt = std::make_unique<HFIPivots<size_t, double>>();
    pvt->setSeed(189);
    ...
    assert(pvt->getPivot(2).getOID() == 5);
    assert(pvt->getPivot(3).getOID() == 21);
#else
    auto data = std::make_unique<Dataset<size_t, double>>("../../data/Dataset1.csv", 20, 2);
    for (size_t i = 0; i < data->getCardinality(); i++)
        data->operator[](i).setOID(data->operator[](i).getOID()+3);
    ...
    pvt->setPivotSize(0.3);
    pvt->operator()(data, dist, 4);
    assert(pvt->getPivot(0)->getOID() == 17);
    assert(pvt->getPivot(1)->getOID() == 10);
    assert(pvt->getPivot(2)->getOID() == 4);
    assert(pvt->getPivot(3)->getOID() == 6);
#endif
...
int test3()
{
    ...
    assert(pvt->getPivot(0).getOID() == 5);
    assert(pvt->getPivot(1).getOID() == 21);
#else
    auto data = std::make_unique<Dataset<size_t, double>>("../../data/Dataset1.csv", 20, 2);
    for (size_t i = 0; i < data->getCardinality(); i++)
        data->operator[](i).setOID(data->operator[](i).getOID()+3);
    ...
    pvt->setNumberOfDropPivots(2);
    pvt->operator()(data, dist, 2);
    assert(pvt->getPivot(0)->getOID() == 4);
