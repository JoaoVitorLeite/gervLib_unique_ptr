...
int test1()
{
    ...
    auto pvt = std::make_unique<ISPivots<size_t, double>>();
    pvt->setSeed(101);
    pvt->operator()(data, dist, 4);
    std::unique_ptr<u_char[]> arr = pvt->serialize();
    std::unique_ptr<Pivot<size_t, double>> pvt2 = std::make_unique<ISPivots<size_t, double>>();
...
int test2()
{
    ...
    pvt->setLimInfSampleSize(0.25);
    pvt->operator()(data, dist, 4);
    assert(pvt->getPivot(0).getOID() == 19);
    assert(pvt->getPivot(1).getOID() == 1);
    assert(pvt->getPivot(2).getOID() == 0);
    assert(pvt->getPivot(3).getOID() == 5);
#endif
