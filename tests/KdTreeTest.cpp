...
int test3()
{
    ...
    std::unique_ptr<kdtree::KdTree<size_t, double>> vp = std::make_unique<kdtree::KdTree<size_t, double>>(std::move(data1), std::move(dist1), std::move(pvt), 2, 50, 4096, false, true, true, "tmp_unit_test19");

    std::unique_ptr<u_char[]> serialized = vp->serialize();
    std::unique_ptr<Index<size_t, double>> deserialized = std::make_unique<kdtree::KdTree<size_t, double>>();
    deserialized->deserialize(std::move(serialized));
...
int test4()
{
    ...
    std::unique_ptr<kdtree::KdTree<size_t, double>> kd = std::make_unique<kdtree::KdTree<size_t, double>>(std::move(data1), std::move(dist1), std::move(pvt), 2, 50, 8000, false, true, true, "tmp_unit_test20");
    ...
    {
        std::vector<gervLib::query::ResultEntry<size_t>> res1 = kd->kNNIncremental(test->getElement(i), 100, true, true);
