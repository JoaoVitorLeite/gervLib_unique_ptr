...
int test1()
{
    ...
    std::unique_ptr<Pivot<size_t, double>> pvt2 = std::make_unique<KmedoidsPivots<size_t, double>>();
...
int test2()
{
    ...
    assert(pvt->getPivot(3).getOID() == 12);
#else
    auto data = std::make_unique<Dataset<size_t, double>>("../../data/Dataset1.csv", 20, 2);
    std::unique_ptr<DistanceFunction<BasicArrayObject<size_t, double>>> dist = std::make_unique<EuclideanDistance<BasicArrayObject<size_t, double>>>();
    auto pvt = std::make_unique<KmedoidsPivots<size_t, double>>();
    ...
    assert(pvt->getPivot(1).getOID() == 4);
    assert(pvt->getPivot(2).getOID() == 6);
    assert(pvt->getPivot(3).getOID() == 17);
#endif
...
int test3()
{
    ...
    assert(pvt->getPivot(1).getOID() == 12);
#else
    auto data = std::make_unique<Dataset<size_t, double>>("../../data/Dataset1.csv", 20, 2);
    std::unique_ptr<DistanceFunction<BasicArrayObject<size_t, double>>> dist = std::make_unique<EuclideanDistance<BasicArrayObject<size_t, double>>>();
    auto pvt = std::make_unique<KmedoidsPivots<size_t, double>>();
    ...
    assert(pvt->getPivot(1).getOID() == 17);
#endif
