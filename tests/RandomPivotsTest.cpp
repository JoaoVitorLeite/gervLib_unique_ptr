...
int test1()
{
    ...
    std::unique_ptr<Pivot<size_t, double>> pvt2 = std::make_unique<RandomPivots<size_t, double>>();
...
int test2()
{
    ...
    auto data = std::make_unique<Dataset<size_t, double>>("../../data/Dataset1.csv", 20, 2);
    auto pvt = std::make_unique<RandomPivots<size_t, double>>();
    std::unique_ptr<DistanceFunction<BasicArrayObject<size_t, double>>> dist;
    pvt->setSeed(156);
    pvt->operator()(data, dist, 4);
    assert(pvt->getPivot(0).getOID() == 3);
    assert(pvt->getPivot(1).getOID() == 5);
    assert(pvt->getPivot(2).getOID() == 7);
    assert(pvt->getPivot(3).getOID() == 15);
#else
    auto data = std::make_unique<Dataset<size_t, double>>("../../data/Dataset1.csv", 20, 2);
    auto pvt = std::make_unique<RandomPivots<size_t, double>>();
    std::unique_ptr<DistanceFunction<BasicArrayObject<size_t, double>>> dist;
...
int test3()
{
    ...
    assert(pvt->getPivot(1).getOID() == 15);
#else
    auto data = std::make_unique<Dataset<size_t, double>>("../../data/Dataset1.csv", 20, 2);
    auto pvt = std::make_unique<RandomPivots<size_t, double>>();
    std::unique_ptr<DistanceFunction<BasicArrayObject<size_t, double>>> dist;
