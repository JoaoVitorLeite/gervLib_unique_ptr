...
int test1()
{
    ...
    auto pvt = std::make_unique<SSSPivots<size_t, double>>();
    pvt->setSeed(446);
    pvt->operator()(data, dist, 4);
    std::unique_ptr<u_char[]> arr = pvt->serialize();
    std::unique_ptr<Pivot<unsigned long, double>> pvt2 = std::make_unique<SSSPivots<size_t, double>>();
...
int test2()
{
    ...
    pvt->setAlpha(0.5);
    pvt->operator()(data, dist, 4);
    assert(pvt->getPivot(0).getOID() == 8);
    assert(pvt->getPivot(1).getOID() == 6);
    assert(pvt->getPivot(2).getOID() == 9);
    assert(pvt->getPivot(3).getOID() == 7);
#else
    auto data = std::make_unique<Dataset<size_t, double>>("../../data/Dataset1.csv", 20, 2);
    std::unique_ptr<DistanceFunction<BasicArrayObject<size_t, double>>> dist = std::make_unique<EuclideanDistance<BasicArrayObject<size_t, double>>>();
    auto pvt = std::make_unique<SSSPivots<size_t, double>>();
    ...
    assert(pvt->getPivot(1).getOID() == 0);
    assert(pvt->getPivot(2).getOID() == 7);
...
int test3()
{
    ...
    assert(pvt->getPivot(0).getOID() == 9);
